...
impl <'a> Doc {
    pub fn on_update (&'a self, observer: std::rc::Weak<impl Subscriber<events::UpdateEvent> + 'static>) {
...
impl <'a> DocInner {
    ...
    pub fn read_structs (&mut self, update_decoder: &mut encoding::UpdateDecoder) {
        ...
        for _ in 0..number_of_clients {
            ...
            for _ in 0..number_of_structs {
                ...
                let client_struct_list = self.ss.get_client_structs_list_with_capacity(client, number_of_structs as usize);
    ...
    pub fn write_structs (&self, update_encoder: &mut encoding::UpdateEncoder, sv: &StateVector) {
        ...
        let mut structs: Vec<(&u32, &ClientBlockList)> = self.ss.clients.iter().filter(|(client_id, sl)| sv.get_state(**client_id) < sl.get_state()).collect();
