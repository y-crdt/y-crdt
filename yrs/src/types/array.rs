...
pub struct ArrayEvent(Rc<RefCell<ArrayEventInner>>);

struct ArrayEventInner {
    target: Array,
    current_target: BranchPtr,
    change_set: Option<Box<ChangeSet<Change>>>,
}

impl ArrayEvent {
    pub(crate) fn new(branch_ref: BranchPtr) -> Self {
        let current_target = branch_ref.clone();
        ArrayEvent(Rc::new(RefCell::new(ArrayEventInner {
    ...
    pub fn path(&self) -> Path {
        let inner = self.0.deref().borrow();
        Branch::path(inner.current_target, inner.target.0)
    }
    ...
    fn changes(&self, txn: &Transaction) -> &ChangeSet<Change> {
        let mut inner = self.0.deref().borrow_mut();
        let change_set = &mut inner.change_set;
        change_set.get_or_insert_with(|| Box::new(event_change_set(txn, inner.target.0.start)))
    }
}
