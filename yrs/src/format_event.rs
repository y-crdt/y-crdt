...
impl FormatEvent {
    ...
    pub fn inspect<T: ReadTxn>(
        txn: &T,
        bytes: &[u8],
        attribute: &str,
        targets: &[FormatTarget],
        max_steps: usize,
    ) -> Result<Self, FormatEventError> {
        if bytes.is_empty()
        ...
        {
            return Err(FormatEventError::Prestate);
        }
        ...
        for (client, ranges) in update.delete_set.iter() {
            for range in ranges.iter() {
                if range.end - range.start > 4096
                    || deleted.len() + (range.end - range.start) as usize > 4096
                {
                    return Err(FormatEventError::Limit);
                }
                for clock in range.clone() {
                    ...
                    if !new.contains(&id) {
                        let item = txn
                            ...
                            .blocks
                            .get_item(&id)
                            .ok_or(FormatEventError::Unprovable)?;
                        if item.is_deleted()
                            || item.len != 1
                            || !matches!(&item.content,ItemContent::Format(key,_) if key.as_ref()==attribute)
                        {
        ...
        if inserted.is_empty() && deleted.is_empty() {
            return Err(FormatEventError::Ineffective);
        }
        let checkpoint = txn.encode_state_as_update_v1(&StateVector::default());
        if checkpoint.len() > 8 * 1024 * 1024 {
            return Err(FormatEventError::Limit);
        }
        ...
        });
        {
            ...
            t.apply_update(Update::decode_v1(&checkpoint).map_err(|_| FormatEventError::Update)?)
                .map_err(|_| FormatEventError::Update)?;
            t.apply_update(update)
                .map_err(|_| FormatEventError::Update)?;
        }
        let t = scratch.transact();
        if t.store().pending_update().is_some() || t.store().pending_ds().is_some() {
            return Err(FormatEventError::Update);
        }
        ...
        let changes = targets
            ...
            .zip(before)
            .zip(after)
            .filter_map(|((target, before), after)| {
                if before == after {
                    None
                } else {
                    Some(FormatChange {
...
fn step(budget: &mut usize) -> Result<(), FormatEventError> {
    *budget = budget.checked_sub(1).ok_or(FormatEventError::Limit)?;
    Ok(())
}
fn observe<T: ReadTxn>(
    txn: &T,
    attribute: &str,
    targets: &[FormatTarget],
    budget: &mut usize,
) -> Result<Vec<FormatState>, FormatEventError> {
    ...
    for (id, wanted) in branches {
        ...
        if !matches!(
            branch.type_ref(),
            TypeRef::Text | TypeRef::XmlText | TypeRef::Undefined
        ) {
            return Err(FormatEventError::Target);
        }
        ...
        while let Some(item) = next {
            ...
            match &item.content {
                ItemContent::Format(key, v) if key.as_ref() == attribute => {
                    marker = Some(item.id);
                    value = (**v).clone();
                }
                ItemContent::String(_) => {
                    for clock in item.id.clock..item.id.clock + item.len {
                        step(budget)?;
                        let unit = ID::new(item.id.client, clock);
    ...
    targets
        ...
        .map(|target| {
            found
                .get(&(target.branch.clone(), target.item))
                .cloned()
                .ok_or(FormatEventError::Target)
        })
        .collect()
}
...
mod tests {
    ...
    fn setup() -> (Doc, crate::TextRef, Vec<FormatTarget>) {
        ...
        let targets = {
            ...
            (0..5)
                .map(|i| FormatTarget {
                    branch: BranchID::Root(Arc::from("text")),
                    item: *text
    ...
    fn additions_removals_and_aba_retain_real_marker_ids() {
        ...
        for value in [Any::Bool(true), Any::Null, Any::Bool(true), Any::Null] {
            let bytes = {
                ...
                text.format(&mut txn, 0, 5, [(Arc::from("bold"), value)].into());
                txn.encode_update_v1()
            };
            let before = receiver
                .transact()
                .encode_state_as_update_v1(&StateVector::default());
            let event = FormatEvent::inspect(&receiver.transact(), &bytes, "bold", &targets, 10000)
                .unwrap();
            assert_eq!(
                receiver
                    .transact()
    ...
    fn malformed_bounds_duplicate_targets_and_nonformat_updates_refuse() {
        let (doc, text, targets) = setup();
        assert_eq!(
            FormatEvent::inspect(&doc.transact(), &[255], "bold", &targets, 1000),
            Err(FormatEventError::Update)
        );
        assert_eq!(
            FormatEvent::inspect(&doc.transact(), &[0, 0], "bold", &targets, 0),
            Err(FormatEventError::Limit)
        );
        assert_eq!(
            FormatEvent::inspect(
                &doc.transact(),
        ...
        let before = Doc::with_options(Options {
            offset_kind: OffsetKind::Utf16,
            ..Options::default()
        });
        before
        ...
        assert_eq!(
            FormatEvent::inspect(&before.transact(), &bytes, "bold", &targets, 1000),
            Err(FormatEventError::Unprovable)
        );
    }
    #[test]
    fn deleted_xml_ancestor_refuses_target() {
        use crate::{SharedRef, XmlElementPrelim, XmlFragment, XmlTextPrelim};
        let doc = Doc::with_options(Options {
            offset_kind: OffsetKind::Utf16,
            skip_gc: true,
            ..Options::default()
        ...
        let text = {
            let mut txn = doc.transact_mut();
            let p = root.push_back(&mut txn, XmlElementPrelim::empty("p"));
            p.push_back(&mut txn, XmlTextPrelim::new("Hello"))
        };
        let target = {
            let txn = doc.transact();
            FormatTarget {
                branch: text.hook().id().clone(),
                item: *text
        ...
        assert_eq!(
            FormatEvent::inspect(&doc.transact(), &[0, 0], "bold", &[target], 1000),
            Err(FormatEventError::Target)
        );
    }
    ...
    fn actual_yjs_format_event_parity() {
        ...
        let fixture: Value =
            serde_json::from_str(include_str!("fixtures/format_event_js_v1.json")).unwrap();
        for case in fixture["cases"].as_array().unwrap() {
            ...
            doc.transact_mut()
                .apply_update(Update::decode_v1(&bytes(&case["before"])).unwrap())
                .unwrap();
            let expected = &case["event"];
            let targets: Vec<_> = expected["changes"]
                .as_array()
                .unwrap()
                .iter()
                .map(|c| FormatTarget {
                    branch: BranchID::Nested(id(&c["unit"]["type"])),
                    item: id(&c["unit"]["item"]),
                })
                .collect();
            let event = FormatEvent::inspect(
                &doc.transact(),
                &bytes(&case["update"]),
                "bold",
                &targets,
            ...
            .unwrap();
            for (actual, key) in [
                (&event.inserted, "inserted_markers"),
                (&event.deleted, "deleted_markers"),
            ] {
                let mut ids: Vec<_> = expected[key].as_array().unwrap().iter().map(id).collect();
                ids.sort();
                assert_eq!(*actual, ids);
            }
            assert_eq!(event.changes.len(), targets.len());
            for (actual, expected) in event
                .changes
                .iter()
                .zip(expected["changes"].as_array().unwrap())
            {
                for (state, key) in [(&actual.before, "before"), (&actual.after, "after")] {
                    ...
                    if expected[key]["enabled"].as_bool().unwrap() {
                        assert_eq!(state.value, Any::Map(Arc::new(Default::default())));
                    } else {
                        assert_eq!(state.value, Any::Null);
                    }
                }
            }
        }
