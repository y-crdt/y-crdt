...
pub fn merge_updates_v1(updates: &[&[u8]]) -> Vec<u8> {
    let x: Vec<Update> = updates
        .iter()
        .map(|buf| Update::decode_v1(buf))
        .into_iter()
        .collect();
    Update::merge_updates(x).encode_v1()
}

pub fn merge_updates_v2(updates: &[&[u8]]) -> Vec<u8> {
    let x: Vec<Update> = updates
        .iter()
        .map(|buf| Update::decode_v2(buf))
        .into_iter()
        .collect();
    Update::merge_updates(x).encode_v2()
}

// Computes the state vector from a document update
pub fn encode_state_vector_from_update_v1(update: &[u8]) -> Vec<u8> {
    let update = Update::decode_v1(update);
    update.state_vector().encode_v1()
}

// Computes the state vector from a document update
pub fn encode_state_vector_from_update_v2(update: &[u8]) -> Vec<u8> {
    let update = Update::decode_v2(update);
    update.state_vector().encode_v2()
}

// Encode the missing differences to another document update.
pub fn diff_updates_v1(update: &[u8], state_vector: &[u8]) -> Vec<u8> {
...
// Encode the missing differences to another document update.
pub fn diff_updates_v2(update: &[u8], state_vector: &[u8]) -> Vec<u8> {
...
mod test {
    ...
    fn merge_updates_compatibility_v1() {
        ...
        let actual = merge_updates_v1(&[a, b]);
    ...
    fn encode_state_vector_compatibility_v1() {
        ...
        let actual = encode_state_vector_from_update_v1(update);
        assert_eq!(actual, expected);
    }
    ...
    fn diff_updates_compatibility_v1() {
        ...
        let actual = diff_updates_v1(update, state_vector);
        assert_eq!(actual, expected);
    }
}
