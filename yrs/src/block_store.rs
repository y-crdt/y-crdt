...
impl BlockStore {
    ...
    pub fn get_item_mut (&mut self, ptr: &BlockPtr) -> &mut Item {
        if ptr.id.client == self.client_id {
            unsafe {
                self.local_block_list.list.get_unchecked_mut(ptr.pivot as usize)
            }
        } else {
            unsafe {
                ...
                self.clients.get_mut(&ptr.id.client).unwrap().list.get_unchecked_mut(ptr.pivot as usize)
            }
        }
    }
    #[inline(always)]
    ...
    pub fn get_client_structs_list (&mut self, client_id: u32) -> &mut ClientBlockList {
        if client_id == self.client_id {
            ...
            self.clients.entry(client_id).or_insert_with(ClientBlockList::new)
        }
    }
    #[inline(always)]
    pub fn get_client_structs_list_with_capacity (&mut self, client_id: u32, capacity: usize) -> &mut ClientBlockList {
        if client_id == self.client_id {
            &mut self.local_block_list
        } else {
            self.clients.entry(client_id).or_insert_with(||ClientBlockList::with_capacity(capacity))
        }
    }
}
