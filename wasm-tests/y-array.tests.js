...
export const testBasicUpdate = tc => {
    const doc1 = new Y.YDoc()
    const doc2 = new Y.YDoc()
    doc1.transact((txn) => txn.getArray('array').insert(txn, 0, ['hi']));
    const update = Y.encodeStateAsUpdate(doc1)
    Y.applyUpdate(doc2, update)
    t.compare(doc2.transact((txn) => txn.getArray('array').toJson(txn)), ['hi'])
}
...
export const testSlice = tc => {
    const doc1 = new Y.YDoc()
    const txn = doc1.beginTransaction();
    const arr = txn.getArray('array')

    arr.insert(txn,0, [1, 2, 3])

    var s = arr.toJson(txn)
    t.compareArrays(s.slice(0), [1, 2, 3])
    t.compareArrays(s.slice(1), [2, 3])
    t.compareArrays(s.slice(0, -1), [1, 2])

    arr.insert(txn,0, [0])

    s = arr.toJson(txn)
    t.compareArrays(s.slice(0), [0, 1, 2, 3])
    t.compareArrays(s.slice(0, 2), [0, 1])
...
export const testLengthIssue = tc => {
    ...
    const txn = doc1.beginTransaction();
    const arr = txn.getArray('array')
    arr.push(txn,[0, 1, 2, 3])
    arr.delete(txn,0, 1)
    arr.insert(txn, 0, [0])
    t.assert(arr.length === arr.toJson(txn).length)
